...
export const Header = ({
  ...
  return (
    <header className="fixed z-30 flex w-full items-center justify-between bg-gray-100 px-3 py-[18px] dark:border-b-2 dark:border-b-accent-base dark:bg-gray-800 md:px-10">
      <div className="relative hidden md:flex lg:hidden">
        ...
        {menu && (
          <ul className="absolute flex flex-row items-start gap-4 rounded-md bg-gray-100 p-6 dark:border-2 dark:border-accent-base dark:bg-gray-800">
            <div className="flex flex-col">
              {navigation.map(({ name, darkModeIcon, lightModeIcon }) => (
                <li key={name}>
                  <a className="group flex flex-row gap-2" href={`#${name}`}>
                    {theme === "dark" ? darkModeIcon : lightModeIcon}
                    <p className="text-gray-50 group-hover:border-b-2 group-hover:border-gray-50 dark:text-gray-300 dark:group-hover:border-b-2 dark:group-hover:border-gray-100 md:text-xl">
                      {name}
                    </p>
                  </a>
                </li>
      ...
      <div className="flex items-center gap-4">
        <ul className="flex flex-row items-start gap-4 dark:bg-gray-800 max-[1023px]:hidden">
          {navigation.map(({ name, darkModeIcon, lightModeIcon }) => (
            <li key={name}>
              <a
        ...
        <div className="flex h-full gap-1">
          <a
            href="https://www.instagram.com/vwo_solucoes/"
            className="flex shrink-0 cursor-pointer p-1 hover:brightness-125"
          >
            <Image
              src="/instagram.png"
              width={44}
          ...
          <a
            onClick={() => {
              // @ts-expect-error
              gtag_report_conversion("https://wa.me/5511955562955");
            }}
